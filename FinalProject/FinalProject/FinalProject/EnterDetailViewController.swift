...
class EnterDetailViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func ButtonAccount(_ sender: Any) {
        ...
        let lHistoryData = try! Data(contentsOf: myDataFileURL)
        let lHistory = NSKeyedUnarchiver.unarchiveObject(with: lHistoryData) as! Model
        
        myModel?.money = lHistory.money
        myModel?.detail = lHistory.detail
